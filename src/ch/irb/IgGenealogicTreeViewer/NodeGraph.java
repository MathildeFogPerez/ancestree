...
public class NodeGraph extends NodeObject {
	...
	/**
	 * @param xPositionForTheParent
	 *            the xPositionForTheParent to set
	 */
	...
	/**
	 * @param coordinatesProcessed
	 *            the coordinatesProcessed to set
	 */
	...
	public void processYcoordinates(float yDistanceWithParent, int heightNodeShape) {
		...
		if (getEC50() != null || (getComment1() != null && isShowComment1()))
			height *= 1.8;
		setHeight(height);
		// logger.warn("For node "+getNodeId()+" height is "+getHeight());

	// logger.fatal("For node " + getNodeId() + " the y dist with parent is " + yDistanceWithParent);
		float y = 40f;
		if (yDistanceWithParent == 0) // it is the GL
			setYCoord(y);
		else {
		/*	logger.fatal("Parent "+getParent().getNodeId());
			logger.fatal("coord  "+getParent().getYCoord());
			 logger.fatal("... we add the y coord of the parent "+getParent().getNodeId()+" which is "
			 +getParent().getYCoord());*/
			y = getParent().getYCoord() + getParent().getHeight() + yDistanceWithParent;
			// logger.debug("... y is: " + y);
			// add if hasDuplicatesNodes + (30 x numberOfDuplicatedNodes)
			if (getParent().hasDuplicateNodes()) {
				...
				// TO DELETE y += ((heightOfNodeShape + 2) * getParent().getDuplicatedNodes().size());
			}
			setYCoord(y);
		}
		if (hasDuplicateNodes) {
			// logger.warn("This node has duplicates " + getNodeId());
			...
			for (int i = 0; i < duplicatedNodes.size(); i++) {
				...
				// TO DELETE yForDuplicated += (heightOfNodeShape + 2);
				...
				// logger.warn("For node " + node.getNodeId() + " we set y to " + yForDuplicated);
			}
		}
	}
	...
	public void setRectangle2D(int widthOfShapeNode) {
		...
		if (hasDuplicateNodes()) {
			...
			for (int i = 0; i < duplicatedNodes.size(); i++) {
				...
				// TO DELETE y += node.getHeight()+2; // 32
	...
	/**
	 * @param isADuplicateNode
	 *            the isADuplicateNode to set
	 */
	...
	/**
	 * @param hasDuplicateNodes
	 *            the hasDuplicateNodes to set
	 */
